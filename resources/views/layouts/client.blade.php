    ...
    <nav class="bg-white border-b border-stone-200 sticky top-0 z-50">
        <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <a href="{{ route('client.properties') }}" class="flex items-center gap-2">
                ...
                <span class="text-lg font-bold text-stone-800">Estate<span class="text-teal-600">Flow</span></span>
            </a>

            {{-- Desktop Nav --}}
            <div class="hidden md:flex items-center gap-6 text-sm font-medium">
                <a href="{{ route('client.properties') }}" class="{{ request()->routeIs('client.properties') ? 'text-teal-600' : 'text-stone-500 hover:text-stone-800' }} transition">Browse</a>
                <a href="{{ route('client.account.home') }}" class="{{ request()->routeIs('client.account.home') ? 'text-teal-600' : 'text-stone-500 hover:text-stone-800' }} transition">Dashboard</a>
                <a href="{{ route('client.account.reservation') }}" class="{{ request()->routeIs('client.account.reservation') ? 'text-teal-600' : 'text-stone-500 hover:text-stone-800' }} transition">My Reservation</a>
                <a href="{{ route('client.account.documents') }}" class="{{ request()->routeIs('client.account.documents') ? 'text-teal-600' : 'text-stone-500 hover:text-stone-800' }} transition">Documents</a>
            </div>

            {{-- Right Side --}}
            <div class="hidden md:flex items-center gap-3">
                @php $unread = \App\Models\AppNotification::where('user_id', auth()->id())->where('is_read', false)->count(); @endphp
                <a href="{{ route('client.account.notifications') }}" class="relative text-stone-500 hover:text-teal-600 transition">
                ...
                <div class="flex items-center gap-2 pl-3 border-l border-stone-200">
                    <div class="w-8 h-8 bg-teal-700 rounded-full flex items-center justify-center text-white font-bold text-sm">
