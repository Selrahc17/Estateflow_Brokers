...
<body class="bg-stone-100 font-sans min-h-screen flex items-center justify-center p-6">
    ...
    <div class="w-full max-w-lg bg-white rounded-2xl border border-stone-200 p-8 shadow-sm">
        ...
        <form class="space-y-4" x-data="{ role: 'client' }" action="{{ route('auth.register.post') }}" method="POST">
            ...
            <div>
                ...
                <div class="flex bg-stone-100 rounded-xl p-1">
                    <button type="button" @click="role = 'client'"
                        :class="role === 'client' ? 'bg-white text-teal-800 shadow-sm' : 'text-stone-500'"
                        class="flex-1 py-2 rounded-lg text-sm font-medium transition">Client</button>
                    <button type="button" @click="role = 'agent'"
                        :class="role === 'agent' ? 'bg-white text-teal-800 shadow-sm' : 'text-stone-500'"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="first_name" placeholder="Juan" value="{{ old('first_name') }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
                </div>
                <div>
                    <label class="text-sm text-stone-600 font-medium mb-1 block">Last Name</label>
                    <input type="text" name="last_name" placeholder="dela Cruz" value="{{ old('last_name') }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
                </div>
            </div>

            <div>
                <label class="text-sm text-stone-600 font-medium mb-1 block">Email Address</label>
                <input type="email" name="email" placeholder="you@example.com" value="{{ old('email') }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
            </div>

            <div>
                <label class="text-sm text-stone-600 font-medium mb-1 block">Phone Number</label>
                <input type="text" name="phone" placeholder="+63 912 345 6789" value="{{ old('phone') }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
            </div>
        ...
        <p class="text-center text-sm text-stone-400 mt-5">
            ...
            <a href="{{ route('auth.login') }}" class="text-teal-700 hover:underline font-medium">Sign in</a>
        </p>
