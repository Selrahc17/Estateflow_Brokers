...
<div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
    ...
    <article class="flex h-full flex-col overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm transition hover:shadow-md">
        <div class="relative h-48 shrink-0 bg-stone-200">
            ...
            @endif
            <span class="absolute right-3 top-3 rounded-full px-2.5 py-1 text-[11px] font-medium {{ $property->status === 'available' ? 'bg-green-100 text-green-700' : ($property->status === 'sold' ? 'bg-red-100 text-red-600' : 'bg-stone-100 text-stone-600') }}">
        ...
        <div class="flex flex-1 flex-col p-5">
            ...
            <form action="{{ route('broker.property-lists.terms', $property) }}" method="POST" class="mt-5 space-y-3 border-t border-stone-100 pt-4">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div>
                        <label class="text-[11px] font-medium uppercase tracking-wide text-stone-400">Commission Rate (%)</label>
                        <input type="number" name="agent_commission" min="0" max="100" step="0.01" value="{{ old('agent_commission', $property->agent_commission) }}" class="mt-1 w-full rounded-lg border border-stone-200 px-2.5 py-2 text-sm text-stone-700 focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-200">
                    </div>
                    <div>
                        <label class="text-[11px] font-medium uppercase tracking-wide text-stone-400">Valid Until</label>
                        <input type="date" name="valid_until" value="{{ old('valid_until', $property->valid_until?->format('Y-m-d')) }}" class="mt-1 w-full rounded-lg border border-stone-200 px-2.5 py-2 text-sm text-stone-700 focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-200">
                    </div>
                </div>
                <button type="submit" class="w-full rounded-lg bg-red-600 px-3 py-2.5 text-xs font-medium text-white transition hover:bg-red-700">Save Terms</button>
            </form>
        </div>
    </article>
    @empty
