...
<div class="bg-gradient-to-r from-[#112E3B] to-[#1A6B79] text-white">
    <div class="max-w-6xl mx-auto px-6 py-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div class="flex items-center gap-5">
            <div class="w-16 h-16 bg-teal-600 rounded-2xl flex items-center justify-center text-white text-2xl font-bold shrink-0">
                {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
            </div>
            <div>
                <p class="text-teal-500 text-xs uppercase tracking-widest font-semibold mb-1">Welcome back</p>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        @foreach([
            ...
            ['My Documents','client.account.documents','M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
                ['Find Your Match','client.recommendations','M9 3v2m6-2v2M4 9h16M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z'],
        ...
        <a href="{{ route($nav[1]) }}" class="bg-white rounded-2xl border border-stone-200 p-5 flex flex-col items-center gap-3 hover:shadow-md hover:border-teal-200 transition group text-center">
            <div class="w-12 h-12 bg-stone-100 group-hover:bg-teal-100 rounded-xl flex items-center justify-center transition">
                <svg class="w-6 h-6 text-stone-500 group-hover:text-teal-700 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $nav[2] }}"/></svg>
            </div>
            <p class="text-sm font-semibold text-stone-700 group-hover:text-teal-800 transition">{{ $nav[0] }}</p>
        </a>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-stone-200 overflow-hidden">
            <div class="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
                ...
                <a href="{{ route('client.account.reservation') }}" class="text-xs text-teal-700 hover:underline">View all →</a>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-stone-200 overflow-hidden">
            <div class="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
                ...
                <a href="{{ route('client.account.notifications') }}" class="text-xs text-teal-700 hover:underline">See all →</a>
            </div>
            <div class="divide-y divide-stone-100">
                @forelse($notifications as $notif)
                <div class="flex items-start gap-3 px-5 py-3.5 {{ !$notif->is_read ? 'bg-teal-50/50' : '' }}">
                    <div class="w-2 h-2 rounded-full mt-1.5 shrink-0 {{ $notif->is_read ? 'bg-stone-300' : 'bg-teal-600' }}"></div>
                    <div class="flex-1">
