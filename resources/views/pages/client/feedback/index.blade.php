...
<div class="bg-gradient-to-r from-stone-900 to-amber-900 text-white">
    <div class="max-w-6xl mx-auto px-6 py-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <p class="text-amber-400 text-xs uppercase tracking-widest font-semibold mb-1">Feedback & Ratings</p>
...
<div class="bg-white border-b border-stone-200">
    <div class="max-w-6xl mx-auto px-6 py-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
        @foreach([
            ...
            ['4.9','Broker Rating','text-amber-600'],
        ...
        <div class="xl:col-span-2 space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-stone-200 p-6" x-data="{ rating: 0, hover: 0 }">
                ...
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <button @mouseenter="hover = {{ $i }}" @mouseleave="hover = 0" @click="rating = {{ $i }}" class="transition-transform hover:scale-110">
                        <svg class="w-10 h-10 transition-colors duration-150" :class="(hover || rating) >= {{ $i }} ? 'text-amber-400' : 'text-stone-200'" fill="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-5">
                    ...
                    <div x-data="{ rating: 0, hover: 0 }" class="{{ $i < 5 ? 'pb-5 border-b border-stone-100' : '' }}">
                        <div class="flex items-start justify-between mb-2">
                            ...
                            <span class="text-xs font-semibold text-amber-600 shrink-0 ml-2" x-text="['','Poor','Fair','Good','Very Good','Excellent'][rating] || ''"></span>
                        </div>
                        <div class="flex items-center gap-2">
                            @for($j = 1; $j <= 5; $j++)
                            <button @mouseenter="hover = {{ $j }}" @mouseleave="hover = 0" @click="rating = {{ $j }}" class="transition-transform hover:scale-110">
                                <svg class="w-7 h-7 transition-colors duration-150" :class="(hover || rating) >= {{ $j }} ? 'text-amber-400' : 'text-stone-200'" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                                </svg>
                            </button>
                            @endfor
            ...
            <div class="bg-white rounded-2xl border border-stone-200 p-6" x-data="{ chars: 0, selected: [] }">
                ...
                <div class="mb-5">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <button
                            ...
                            :class="selected.includes('{{ $tag }}') ? 'bg-amber-600 text-white border-amber-600' : 'bg-white text-stone-600 border-stone-200 hover:border-amber-400 hover:text-amber-600'"
                ...
                <div>
                    ...
                    <textarea
                        ...
                        class="w-full border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-700 focus:outline-none focus:ring-2 focus:ring-amber-400 resize-none placeholder:text-stone-300">
                    </textarea>
            ...
            <div class="bg-white rounded-2xl border border-stone-200 p-6" x-data="{ recommend: null }">
                ...
                <div class="grid grid-cols-3 gap-3">
                    ...
                    <button @click="recommend = 'maybe'"
                        :class="recommend === 'maybe' ? 'bg-amber-500 text-white border-amber-500 scale-105' : 'bg-white text-stone-600 border-stone-200 hover:border-amber-400'"
                        class="flex flex-col items-center justify-center gap-2 py-4 rounded-2xl border font-medium text-sm transition">
                        <span class="text-2xl">🤔</span>
                        <span>Maybe</span>
                    </button>
            ...
            <div class="bg-white rounded-2xl border border-stone-200 p-6">
                ...
                <div class="flex gap-3 flex-wrap">
                    @for($p = 0; $p < 4; $p++)
                    <div class="w-20 h-20 border-2 border-dashed border-stone-200 rounded-xl flex items-center justify-center hover:border-amber-400 cursor-pointer transition group">
                        <svg class="w-6 h-6 text-stone-300 group-hover:text-amber-400 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                    </div>
                    @endfor
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <button class="flex-1 bg-amber-600 hover:bg-amber-700 text-white py-3.5 rounded-2xl font-semibold text-sm transition flex items-center justify-center gap-2 shadow-sm">
                    ...
                    <div class="p-3 bg-stone-50 rounded-xl border border-stone-100">
                        <div class="flex items-center justify-between mb-1.5">
                            ...
                            <div class="flex items-center gap-0.5">
                                @for($s = 1; $s <= 5; $s++)
                                <svg class="w-3 h-3 {{ $s <= (int)$prev[1] ? 'text-amber-400' : 'text-stone-200' }}" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                                @endfor
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="w-20 bg-stone-100 rounded-full h-1.5 overflow-hidden">
                                <div class="bg-amber-400 h-1.5 rounded-full" style="width: {{ (float)$cat[1]/5*100 }}%"></div>
                            </div>
                            <span class="text-xs font-bold text-amber-600 w-6">{{ $cat[1] }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="bg-white rounded-2xl border border-stone-200 p-5">
                ...
                <div class="space-y-4">
                    ...
                    <div class="pb-4 border-b border-stone-100 last:border-0 last:pb-0">
                        <div class="flex items-center justify-between mb-1.5">
                            <div class="flex items-center gap-2">
                                <div class="w-7 h-7 bg-amber-100 rounded-full flex items-center justify-center text-amber-700 font-bold text-xs">{{ strtoupper(substr($rev[0],0,1)) }}</div>
