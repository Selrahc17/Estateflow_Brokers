        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-stone-200 p-6 text-center">
                <div class="relative inline-block mb-4">
                    ...
                    @else
                        <div class="w-20 h-20 bg-amber-600 rounded-2xl flex items-center justify-center text-white text-3xl font-bold mx-auto">
                            {{ strtoupper(substr($user->name, 0, 1)) }}
                        </div>
                    @endif
                    <label for="avatar-upload" class="absolute -bottom-1 -right-1 w-7 h-7 bg-amber-600 rounded-full flex items-center justify-center border-2 border-white hover:bg-amber-700 transition cursor-pointer">
                ...
                <div class="bg-white rounded-2xl border border-stone-200 p-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="first_name" value="{{ old('first_name', $client->first_name ?? explode(' ', $user->name)[0]) }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 text-stone-700">
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-stone-500 uppercase tracking-widest mb-1.5 block">Last Name</label>
                            <input type="text" name="last_name" value="{{ old('last_name', $client->last_name ?? (explode(' ', $user->name, 2)[1] ?? '')) }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 text-stone-700">
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-stone-500 uppercase tracking-widest mb-1.5 block">Email Address</label>
                            <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 text-stone-700">
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-stone-500 uppercase tracking-widest mb-1.5 block">Phone Number</label>
                            <input type="text" name="phone" value="{{ old('phone', $client->phone ?? $user->phone) }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 text-stone-700">
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-stone-500 uppercase tracking-widest mb-1.5 block">Date of Birth</label>
                            <input type="date" name="date_of_birth" value="{{ old('date_of_birth', $client?->date_of_birth?->format('Y-m-d')) }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 text-stone-700">
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-stone-500 uppercase tracking-widest mb-1.5 block">Civil Status</label>
                            <select name="civil_status" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 text-stone-700">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="address" value="{{ old('address', $client->address ?? '') }}" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 text-stone-700">
                        </div>
                    </div>
                    <button type="submit" class="mt-5 bg-amber-600 hover:bg-amber-700 text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition">
                        Save Changes
                    </button>
                </div>
            </form>
                ...
                <div class="bg-white rounded-2xl border border-stone-200 p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="password" name="current_password" placeholder="••••••••" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400">
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
