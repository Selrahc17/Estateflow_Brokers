    ...
    <form method="POST" action="{{ route('agent.lots.update', $lot) }}">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full border border-stone-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">{{ old('description', $lot->description) }}</textarea>
            </div>
        </div>
        <div class="flex items-center gap-3 mt-6 pt-6 border-t border-stone-200">
            <button type="submit" class="bg-teal-700 hover:bg-teal-800 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition">Update Lot</button>
