...
<div class="max-w-2xl mx-auto bg-white rounded-xl border border-stone-200 p-6">
    <form action="{{ route('agent.site-visits.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="scheduled_at" class="w-full border border-stone-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" required value="{{ old('scheduled_at') }}">
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-stone-700 mb-1">Notes</label>
            <textarea name="notes" class="w-full border border-stone-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" rows="3">{{ old('notes') }}</textarea>
        </div>
